...
export const deleteByGoogleBot = async (batchMax: number, batchSize: number, month: number): Promise<number | Error> => {
    try {
        ...
        while (deletedCount < MAX_DAILY) {
            const consulta = `WITH registros AS (
                                    ...
                                    del_rato AS (
                                        ...
                                        RETURNING r.link_visita_acesso_id
                                    )
